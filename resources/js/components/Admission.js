...
class Admission extends Component{
  ...
  componentDidMount() {
    axios.get('http://localhost:4000/api/admissions/')
      .then(res => {
        this.setState({
            admissions: res.data
        });
...
onSubmitUpdate(e) {
  ...
  console.log(expense);
  console.log(this.state.update);
  axios.post('http://localhost:4000/api/admissions/' + this.state.update, expense)
      .then((res) => {
...
onSubmit(e) {
  ...
  console.log(expense);
  axios.post('http://localhost:4000/api/admissions', expense)
...
deleteExpense(id){
  if (window.confirm("Are you sure?")) {
    axios.delete('http://localhost:4000/api/admissions/'+id)
    .then((res) => {
      console.log(res.data)
    }).catch((error) => {
        console.log(error)
...
<Router>  
    ...
    return(
      ...
      <td className="py-1">
      <p className="testtitle">{admission.admission_content}</p>
      </td>
      <td className="py-1">
      <button className="editbutton" onClick={()=>this.handleModal2(admission.id)}>Edit</button>
      <button type="submit" onClick={()=>this.deleteExpense(admission.id)} className="deletebutton">Delete</button>
     ...
     {this.state.admissions.map((admission, i) => {
       if(admission.id===this.state.update){
  ...
  <Form.Group controlId="email">
    <Form.Label>Nội dung</Form.Label>
          <Form.Control as="textarea" type="textarea" placeholder={admission.admission_content} onChange={this.onChangeTitleUpdate}/>
  </Form.Group>
